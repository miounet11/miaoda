...
export const useCustomProvidersStore = defineStore(
  'customProviders',
  () => {
    // State
    ...
    const unhealthyProviders = computed(() => providersList.value.filter(p => !p.isHealthy))
    ...
    const startHealthCheckMonitoring = (intervalMs = 300000) => {
      // 5 minutes default
