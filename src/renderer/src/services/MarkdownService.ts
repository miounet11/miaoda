 ...
 */
function validateMarkdownInput(content: string): { valid: boolean; sanitized: string; error?: string } {
  try {
    ...
    if (normalized.length > 1000000) { // 1MB limit
      return {
        valid: false,
  ...
  try {
    ...
    if (options.enableHighlight !== false) {
      try {
          ...
          highlight(code: string, language: string): string {
            try {
              ...
              if (typeof code !== 'string') {
                logger.warn('Non-string code passed to highlighter', 'MarkdownService', { codeType: typeof code })
                return String(code || '')
              }
        ...
        logger.error('Failed to setup syntax highlighting', 'MarkdownService', { error: highlightSetupError })
    ...
    renderer.code = function(code: string, language?: string, escaped?: boolean): string {
      try {
        ...
        if (language !== undefined && typeof language !== 'string') {
          logger.warn('Non-string language in renderer', 'MarkdownService', { languageType: typeof language })
    ...
    renderer.codespan = function(code: string): string {
      try {
        if (typeof code !== 'string') {
          logger.warn('Non-string codespan in renderer', 'MarkdownService', { codeType: typeof code })
          code = String(code || '')
        }
...
class MarkdownService {
  ...
  private getMarkedInstance(options: MarkdownOptions = {}): typeof marked {
    ...
    if (!this.instances.has(optionsKey)) {
      this.instances.set(optionsKey, createIsolatedMarkedInstance({ ...this.defaultOptions, ...options }))
    }
  ...
  parseWithErrorDisplay(rawContent: any, options: MarkdownOptions = {}): string {
    ...
    if (result.error) {
      ...
      return `<div class="error-content p-4 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded-lg">
        <p class="text-red-800 dark:text-red-200 font-medium">⚠️ Markdown 渲染错误</p>
        <p class="text-sm text-red-600 dark:text-red-400 mt-1">${safeHtmlEscape(result.error)}</p>
        <details class="mt-2 cursor-pointer group">
          ...
          <pre class="text-xs text-red-600 dark:text-red-400 mt-2 p-2 bg-red-100 dark:bg-red-900/30 rounded border overflow-auto max-h-40 whitespace-pre-wrap select-text break-all">${safeHtmlEscape(originalContent)}</pre>
        </details>
      </div>`
    }
