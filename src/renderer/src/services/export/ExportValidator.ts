...
export class ExportValidator {
   ...
   */
  static validateExportSize(chatData: any[], format: string): { isValid: boolean; estimatedSize: number; errors: string[] } {
    const errors: string[] = []
    ...
    if (estimatedSize > MAX_SIZE) {
      errors.push(`Estimated export size (${Math.round(estimatedSize / 1024 / 1024)}MB) exceeds maximum limit (100MB)`)
    } else if (estimatedSize > WARN_SIZE) {
      errors.push(`Warning: Large export size (${Math.round(estimatedSize / 1024 / 1024)}MB) may cause performance issues`)
    }
