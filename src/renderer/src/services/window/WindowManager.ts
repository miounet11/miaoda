...
export class WindowManager extends EventEmitter<{
  ...
  private async confirmCloseWindow(window: WindowState): Promise<boolean> {
    ...
    if (window.api?.dialog?.showMessageBox) {
      const response = await window.api.dialog.showMessageBox({
        ...
        message: 'You have unsaved changes in some tabs. Are you sure you want to close this window?',
  ...
  moveTabToWindow(sourceWindowId: string, tabId: string, targetWindowId: string, index?: number): boolean {
  ...
  async setWindowBounds(windowId: string, bounds: Partial<WindowState['bounds']>): Promise<boolean> {
    const window = this.windows.get(windowId)
    if (!window) return false
