...
describe('Time Utilities', () => {
  ...
  describe('Edge Cases and Performance', () => {
    ...
    it('performs efficiently with multiple calls', () => {
      const timestamps = Array(1000).fill(null).map((_, i) => 
        mockDate.getTime() - i * 60000 // Every minute for 1000 minutes
      )
    ...
    it('handles concurrent calls safely', async () => {
      const promises = Array(100).fill(null).map((_, i) => 
        Promise.resolve(formatDistanceToNow(mockDate.getTime() - i * 1000))
      )
