...
export function createErrorDisplay(error: string, originalContent: any): string {
  ...
  return `<div class="markdown-error-container p-4 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded-lg" style="contain: layout;">
    <div class="flex items-start gap-2 mb-2">
      ...
      <div class="flex-1">
        <p class="text-red-800 dark:text-red-200 font-medium text-sm">Markdown 渲染错误</p>
        <p class="text-xs text-red-600 dark:text-red-400 mt-1">TypeError: value.replace is not a function</p>
        <p class="text-xs text-red-500 dark:text-red-400 mt-1">Please report this to https://github.com/markedjs/marked.</p>
      </div>
    </div>
    <details class="mt-3 cursor-pointer group" style="contain: layout;" id="${errorId}">
      <summary class="text-sm text-red-700 dark:text-red-300 hover:text-red-900 dark:hover:text-red-100 transition-colors cursor-pointer select-none font-medium outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 rounded px-1 py-0.5 hover:bg-red-100 dark:hover:bg-red-900/20 inline-flex items-center gap-1">
      ...
      <div class="mt-2 p-3 bg-red-100 dark:bg-red-900/30 rounded border border-red-200 dark:border-red-800" style="contain: layout;">
        <pre class="text-xs text-red-600 dark:text-red-400 overflow-auto max-h-40 whitespace-pre-wrap select-text break-all font-mono leading-relaxed">${escapeHtml(safeOriginalContent)}</pre>
      </div>
    </details>
  </div>`
}
