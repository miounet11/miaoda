...
export function useEnhancedShortcuts() {
    ...
    if (shortcut && shortcut.action) {
      ...
      if (shortcut.global || shouldHandleShortcut(event.target as HTMLElement)) {
        ...
        try {
          ...
          logger.error(
            `Shortcut action failed: ${error instanceof Error ? error.message : String(error)}`
          )
        }
      }
    }
  ...
  const shouldHandleShortcut = (target: HTMLElement): boolean => {
    ...
    if (
      ...
      // 在输入框中只允许特定的快捷键
      return ['global-search', 'send-message', 'new-line'].some(
        id =>
          shortcutMap.get(
            generateShortcutKey({
              key: '',
  ...
  const loadFromStorage = () => {
    try {
      ...
      logger.error(
        `Failed to load shortcuts from storage: ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
  ...
  const saveToStorage = () => {
    try {
      ...
      logger.error(
        `Failed to save shortcuts to storage: ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
