  ...
  scrollTimer.value = setTimeout(() => {
    ...
    if (lastAnnouncedIndex.value !== startIndex.value) {
      announceToScreenReader(
        `Showing items ${startIndex.value + 1} to ${Math.min(endIndex.value + 1, props.items.length)} of ${props.items.length}`
      )
...
const handleKeydown = (event: KeyboardEvent) => {
  ...
  switch (key) {
    ...
    case 'PageDown':
      ...
      navigateToItem(
        Math.min(
          focusedIndex.value + Math.floor(props.containerHeight / props.estimatedItemHeight),
...
const handleTouchEnd = (event: TouchEvent) => {
  ...
  if (
    deltaTime < 300 &&
    (Math.abs(deltaX) > props.swipeThreshold || Math.abs(deltaY) > props.swipeThreshold)
  ) {
    const target = event.target as HTMLElement
