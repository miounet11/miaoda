<template>
  <div v-if="open" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click="$emit('update:open', false)">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-6" @click.stop>
      <div class="flex items-center space-x-3 mb-4">
        ...
        <div>
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">An error occurred while exporting your data</p>
        </div>
      </div>
      ...
      <div class="mb-6">
        <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
          ...
          <p class="text-sm text-red-700 dark:text-red-300 font-mono break-words">{{ error?.message || 'Unknown error occurred' }}</p>
          
          <div v-if="error?.details" class="mt-3 pt-3 border-t border-red-200 dark:border-red-700">
            <h5 class="text-xs font-medium text-red-800 dark:text-red-200 mb-1">Additional Information:</h5>
            <pre class="text-xs text-red-600 dark:text-red-400 whitespace-pre-wrap">{{ error.details }}</pre>
          </div>
        </div>
        
        <!-- Troubleshooting Tips -->
        <div class="mt-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
          <h4 class="text-sm font-medium text-blue-800 dark:text-blue-200 mb-2">Troubleshooting Tips:</h4>
          <ul class="text-sm text-blue-700 dark:text-blue-300 space-y-1">
            ...
            <li v-if="isPermissionError">• Ensure you have write permissions to the download folder</li>
