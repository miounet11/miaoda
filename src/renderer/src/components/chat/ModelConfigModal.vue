<template>
  <Teleport to="body">
    <div class="fixed inset-0 z-60 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div
        class="bg-background border border-border rounded-xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-hidden"
      >
        <!-- 头部 -->
        ...
        <div class="p-6 space-y-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          ...
          <form @submit.prevent="handleSubmit" class="space-y-4">
                ...
                >
                  ...
                  >
                    <path
                      ...
                      stroke-linejoin="round"
                      stroke-width="2"
                      d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L3 3m6.878 6.878L12 12m6-6l2.121 2.121m0 0l-2.121 2.121M21 3l-2.121 2.121"
                    />
                  </svg>
                  <svg
                    v-else
            ...
            <div v-if="provider?.requiresBaseUrl">
              <label class="block text-sm font-medium text-foreground mb-2"> API 端点地址 </label>
              <input
...
const validateForm = () => {
  ...
  if (props.provider?.requiresApiKey) {
    if (!formData.value.apiKey) {
      ...
      props.provider.apiKeyPattern &&
      !props.provider.apiKeyPattern.test(formData.value.apiKey)
    ) {
