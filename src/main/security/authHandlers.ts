...
import { UserAuthService, type LoginRequest, type RegisterRequest, type PasswordResetRequest, type PasswordResetConfirmRequest } from '../security/UserAuthService'
...
export function registerAuthHandlers(userService: UserService) {
   ...
   * User Logout
   */
  ipcMain.handle('auth:logout', async (event, data: { sessionId: string; allDevices?: boolean }) => {
    try {
   ...
   */
  ipcMain.handle('auth:confirm-password-reset', async (event, request: PasswordResetConfirmRequest) => {
    try {
   ...
   * Revoke Session
   */
  ipcMain.handle('auth:revoke-session', async (event, data: { sessionId: string; targetSessionId: string }) => {
    try {
    ...
    try {
      ...
      return {
        user: {
          ...
          preferences: updatedUser!.preferences ? JSON.parse(updatedUser!.preferences) : undefined,
  ...
  ipcMain.handle('auth:change-password', async (event, data: {
    ...
    try {
      ...
      const isCurrentPasswordValid = await userService.verifyPassword(sessionInfo.user.id, currentPassword)
      ...
      return { success: true, message: 'Password changed successfully. Other sessions have been logged out.' }
